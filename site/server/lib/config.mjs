...
export function loadConfig({ requireSecrets = true } = {}) {
  ...
  return {
    ...
    // РУБИЛЬНИК ПРИЁМА ПДн. Пункт 11 «Порядка к запуску» (BACKLOG.md): публичный
    // приём реальных персональных данных открывается только после SSL и после
    // того, как оператор появится в реестре Роскомнадзора (ст. 22 152-ФЗ).
    //
    // ДЕФОЛТ — ВЫКЛЮЧЕНО, и это осознанно: забыли прописать переменную — приём
    // закрыт, а не открыт. Ошибка в сторону молчания стоит нисколько,
    // ошибка в другую сторону — 100–300 тыс. ₽ и предписание.
    intakeEnabled: process.env.INTAKE_ENABLED === '1',
    // Баннер «сайт в режиме разработки». Отдельный флаг: сайт может уже законно
    // принимать данные, но всё ещё дорабатываться. Фраза про сбор ПДн внутри
    // баннера привязана к intakeEnabled и снимается сама.
    // С 04.09.2026 по умолчанию ВЫКЛЮЧЕН: сайт открыт. DEV_NOTICE=1 вернёт плашку.
