 ...
 *
 * Используется двумя местами: кнопкой «Скачать в Word» на /admin/guide и
 * скриптом tools/build-guide.mjs (сборка файла из командной строки).
 */
...
export async function buildGuideDocx() {
...
const doc = new Document({
  ...
  return {
    buf,
    headings: blocks.filter((b) => b.tag === 'h2').length,
    items: blocks.filter((b) => b.tag === 'li').length,
  };
}
