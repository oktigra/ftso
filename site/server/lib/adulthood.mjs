...
// ПРАВИЛА «СНЯТЬ ПУБЛИКАЦИЮ В 18» БОЛЬШЕ НЕТ (ТЗ ред. 6, модель РТТ). Оно
// родилось из логики «согласие давал представитель, в 18 оно кончилось», а
// результаты соревнований публикуются на основании УЧАСТИЯ в них (п. 5 ч. 1
// ст. 6 152-ФЗ, 329-ФЗ), не согласия. В день 18-летия меняется одно:
// управление кабинетом переходит от представителя к самому участнику.
// Рейтинг, результаты и профиль не трогаются — никто не исчезает из таблицы.
...
export function runAdulthoodCheck(db, { baseUrl, now = today() } = {}) {
  ...
  for (const acc of grown) {
    db.transaction(() => {
      ...
      // Публикация НЕ снимается: строка в рейтинге, очки, матчи и профиль
      // остаются как есть (см. комментарий у начала файла).
