...
export function createApp(config) {
  ...
  app.use((req, res, next) => {
    ...
    // 08.09.2026: показывается и в админке тоже — решение владельца «на всех
    // страницах». Раньше админку исключали как «администратор и так знает», но
    // плашка заодно напоминает, что она всё ещё висит и её пора снять.
    res.locals.devNotice = devNoticeOn(db, config);
    next();
  });
