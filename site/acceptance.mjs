...
await check('импорт турнира из текста: сетка с bye/отказом/матчем за 3 место, группа, пары → черновик с игроками, матчами и местами; ошибка не пишет ничего', async () => {
  ...
  const text = ['Турнир: Импортный ветеранский', 'Даты: 2026-08-01 — 2026-08-03', 'Город: Вязьма | Категория: A',
    ...
    'Пары: Мужские пары | пол: M', 'Итог: 1 Импермаков/Имппестов, 2 Импакаев/Импгруздин',
    // Микст ТЕМИ ЖЕ людьми: с 08.09.2026 это отдельная дисциплина. Раньше
    // UNIQUE (турнир, игрок, дисциплина) молча съедал одно из двух парных мест.
    'Пары: Микст | пол: X', 'Итог: 1 Импакаев/Импбукатина, 2 Импермаков/Импадаева'].join('\n');
  ...
  eq(res.filter((r) => r.discipline === 'mixed').map((r) => `${r.place}:${r.n}`).join(','), '1:Импакаев,1:Импбукатина,2:Импадаева,2:Импермаков', 'места микста');
  // Ермаков и Акаев играли и пары, и микст — обе записи должны сохраниться.
  eq(res.filter((r) => r.n === 'Импермаков').map((r) => r.discipline).sort().join(','), 'double,mixed', 'у игрока пропало место: пары и микст должны храниться раздельно');
...
// ---------------------------------------------------------------------------
await check('«Сохранить данные для входа»: чекбокс на /login и /cabinet/login; с отметкой кука живёт ~90 дней, без — 12 часов', async () => {
  ...
  for (const p of ['/login', '/cabinet/login']) {
    ...
    assert(/name="remember"[^>]*value="1"/.test(page.text) && page.text.includes('Сохранить данные для входа на этом устройстве (90 дней)'), `${p}: нет чекбокса «Сохранить данные для входа (90 дней)»`);
  ...
  assert(days(rem) >= 89 && days(rem) <= 91, `с отметкой кука должна жить ~90 дней, а живёт ${days(rem)}`);
  ...
  assert(days(cr) >= 89 && days(cr) <= 91, `кабинет: с отметкой кука должна жить ~90 дней, а живёт ${days(cr)}`);
  resetCabinetLimit();
...
await check('заявка на турнир и анкета тренера тоже под проверкой', async () => {
  ...
  assert(/вы не робот/.test(res.text), 'в ответе нет текста о непройденной проверке «вы не робот»');
